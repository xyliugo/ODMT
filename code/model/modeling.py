...
class CoAttention(BertPreTrainedModel):
    ...
    def forward(
        ...
        lang_att_output = lang_feats + self.token_type_embeddings(
            torch.zeros_like(lang_attention_mask).long().to(local_rank)
        )
        visn_att_output = visn_feats + self.token_type_embeddings(
            torch.ones_like(visn_attention_mask).long().to(local_rank)
        )
        ...
        for co_attn_block in self.CoAtt_layers:
            lang_att_output, visn_att_output = co_attn_block.forward(
                lang_att_output,
                lang_attention_mask,
                visn_att_output,
                visn_attention_mask,
        ...
        # mean
        pad_mask_text = (
            pad_mask_text.unsqueeze(-1).expand(lang_att_output.size()).float()
        )
        pad_mask_CV = pad_mask_CV.unsqueeze(-1).expand(visn_att_output.size()).float()

        lang_output = torch.sum(lang_att_output * pad_mask_text, 1) / torch.clamp(
            pad_mask_text.sum(1), min=1e-9
        )
        visn_output = torch.sum(visn_att_output * pad_mask_CV, 1) / torch.clamp(
            pad_mask_CV.sum(1), min=1e-9
        )
...
class MergedAttention(BertPreTrainedModel):
    def __init__(self, config, args):
        ...
        self.MergeAtt_layers = nn.ModuleList(
            [MergedAttBolck(config) for _ in range(args.merged_att_layers)]
        )
    ...
    def forward(
        ...
        att_output = torch.sum(att_output * pad_mask, 1) / torch.clamp(
            pad_mask.sum(1), min=1e-9
        )
