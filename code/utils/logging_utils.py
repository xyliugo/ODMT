...
def report_time_train(
    ...
    loss /= batch_index
    Log_file.info(
        "epoch: {} end, train_loss: {:.5f}, align: {:.5f}, uniform: {:.5f}".format(
            now_epoch, loss, align, uniform
        )
    )
...
def save_model(
    now_epoch, model, model_dir, optimizer, rng_state, cuda_rng_state, Log_file
):
    ckpt_path = os.path.join(model_dir, f"epoch-{now_epoch}.pt")
    torch.save(
        {
            "model_state_dict": model.module.state_dict(),  # 模型
