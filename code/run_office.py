...
root_data_dir = os.path.abspath(os.path.join(os.path.join(BASE_DIR,'../../..'), 'home/public/lxy'))
root_model_dir = root_data_dir

## dataset
dataset, tag = 'amazon', 'Office_Products'
ds_config = DatasetConfig(dataset, tag)
behaviors, text_data, image_data, text_model_load, max_seq_len = ds_config.get_dataset_config()
...
tau_list, alpha_list = [0.4], [40, 0]
merged_att_layers_list = [2,2]
init_list = [1,0]
                ...
                for embedding_dim in embedding_dim_list:
                        for merged_att_layers, init in zip(merged_att_layers_list, init_list):
                                run_py = "CUDA_VISIBLE_DEVICES='{}' \
                                        ...
                                        gpu_id, len(gpu_id.split(',')), master_port, root_data_dir, root_model_dir, dataset, behaviors, text_data, image_data,
                                        item_tower, load_ckpt_name, label_screen, max_seq_len,
                                        lr, image_model_load, text_model_load, epoch, 
                                        embedding_dim, id_embedding_dim, text_embedding_dim, image_embedding_dim,
                                        version, seed, merged_att_layers, init, alpha, tau)
